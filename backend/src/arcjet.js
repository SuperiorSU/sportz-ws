...
arcjet({
    ...
    rules:[
        ...
        detectBot({mode: arcjetMode, allow: ['CATEGORY:SEARCH_ENGINE', 'CATEGORY:PREVIEW']}), // disallow bots from data scrapping except for search engine and preview category bots, there are many options as well
        slidingWindow({mode: arcjetMode, interval: '2s', max:5}) // rate limiting rule/algorithm that tracks the requests over a moving time frame: Here we are allowing 5 requests max in 2s per IP address
    ]
}): null;
...
export const securityMiddleware =()=>{
    return async(req, res, next)=>{
        ...
        try{
            const decision = await httpArcjet.protect(req); // here the decision will consist a value based on the rules defined in the httpArcjet[customizable]
